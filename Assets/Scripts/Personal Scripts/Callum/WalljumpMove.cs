...
public class WalljumpMove : MonoBehaviour
{
    private Rigidbody2D rb;
    private CapsuleCollider2D[] hitboxes;

    private RaycastHit2D walljump;
    private Vector2 walljump_dir;
    [SerializeField] private float walljump_reach = 5f;
    private int walljump_count = 1;
    ...
    void Start()
    {
        ...
        walljump_dir = new Vector2(walljump_reach, 0);
    }
    ...
    {   // detect input in Update() so we don't miss any inputs
        ...
        {
            walljump_dir = new Vector2(-1 * walljump_reach, 0);
        }

        walljump = Physics2D.Raycast(transform.position, walljump_dir, walljump_reach, wallLayers);
    ...
    private void OnDrawGizmos()
    {
        ...
        if (facing_right)
        {
            Gizmos.DrawRay((Vector2)transform.position, Vector2.right * walljump_reach);
        } else
        {
            Gizmos.DrawRay((Vector2)transform.position, Vector2.left * walljump_reach);
        }
